document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchTransactions(accountNumber) {
        try {
            ...
            const transactionsList = document.getElementById('transactions-lists');
            ...
            const transactionsGroupedByDate = transactions.reduce((group, transaction) => {
                if (transaction.from_user_id === accountNumber) { // Filtrer uniquement les transactions sortantes
            ...
            Object.keys(transactionsGroupedByDate).forEach(date => {
                ...
                transactionsGroupedByDate[date].forEach(transaction => {
                    ...
                    detailsDiv.innerHTML = `
                        <h3>${transaction.beneficiaire}</h3>
                        <p>${transaction.description || 'Pago enviado'} - ${transaction.statut}</p>
                    `;

                    const amountDiv = document.createElement('div');
                    amountDiv.classList.add('all-content-price');
                    amountDiv.innerHTML = `
                        <p class="color-red">- ${transaction.amount} €</p>
                    `;
