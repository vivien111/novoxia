document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!apiToken || !utilisateurId) {
        ...
        .then(({ utilisateur, solde }) => {
            ...
            if (nomPrenomElement) {
                nomPrenomElement.textContent = nomComplet;
            }
            ...
            if (emailElement) {
                emailElement.textContent = utilisateur.email || 'Non spécifié';
            }
            const IbanElement = document.getElementById('iban');
            if (IbanElement) {
                IbanElement.value = utilisateur.iban || 'Non spécifié';
            }
            const BicElement = document.getElementById('bic');
            if (BicElement) {
                BicElement.value = utilisateur.bic || 'Non spécifié';
            }
